...
p_load(
    "tidyverse", "jsonlite", "magrittr", "readxl", "writexl",
    "stargazer", "psych", "jtools", "DT", "ggstatsplot",
    "lavaan",
    "MplusAutomation", "igraph", "shiny", "ggplot2", "tidyLPA",
...
CAMwordlist <- readxl::read_excel(path = "CAMwordlist.xlsx", sheet = 1)
CAMwordlist <- as.data.frame(CAMwordlist) 
...
### load list of translation of basal attributes
setwd("../data final list basal attributes")
listBasalAttributes <- readxl::read_excel(path = "final list basal attributes.xlsx", sheet = 1)
listBasalAttributes <- as.data.frame(listBasalAttributes)
colnames(listBasalAttributes)[1:3] <- c("Basal.Attribute", "Explenation", "German.Translation") 
...
### translate aggregated CAM
# Save the original names to use for matching
original_names <- V(g2)$name

# Create a named vector for translation
name_translation <- setNames(listBasalAttributes$Basal.Attribute, listBasalAttributes$German.Translation)

# Apply the translation while preserving vertex structure
V(g2)$name <- ifelse(original_names %in% names(name_translation),
                     name_translation[original_names],
                     original_names)

# Optionally set a readable label for plotting
V(g2)$label <- V(g2)$name

# Specific override for one label
V(g2)$label[V(g2)$name == "Akzeptanz eines neuen Materialsystems"] <- "Acceptance of a new material system"
...
## save as .xlsx file
writexl::write_xlsx(x = dat_out, path = "LeidenAlgorithm_solution.xlsx")
...
for (i in 1:nrow(partitions_means)) {
    tmp_boolean <- dat_out$partition == i
...
combinations <- combn(partitions_means$partition, 2, simplify = FALSE)
...
for (i in 1:length(combinations)) {
    tmp <- data.frame(
        ...
        wordsA = paste0(dat_out$words[dat_out$partition == combinations[[i]][1]], collapse = " \\ "),
        wordsB = paste0(dat_out$words[dat_out$partition == combinations[[i]][2]], collapse = " \\ ")
    )
...
## save as .xlsx file
writexl::write_xlsx(x = dat_out_hypotheses, path = "LeidenAlgorithm_hypotheses.xlsx")
