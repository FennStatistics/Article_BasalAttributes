...
plot(g2, edge.arrow.size = 0,
     ...
     vertex.label.cex = .9,
     edge.weight=2, edge.width=(E(g2)$weight/45),   vertex.color = node.cols)
```
...
setwd("outputs/02_partII_CAMs/Leiden algorithm aggregated CAM")
...
## save as .xlsx file
xlsx::write.xlsx2(x = dat_out, file = "LeidenAlgorithm_solution.xlsx")
## save as R object
...
Community detection based on the single-layer multi-edge aggregated graph.
Partitions are then pruned with ModularityPruning (http://github.com/ragibson/ModularityPruning)
to keep only stable and modularity-optimal solutions.
...
Community detection based on a multilayer graph with participants as layers.
Partitions are then pruned with ModularityPruning (http://github.com/ragibson/ModularityPruning)
to keep only stable and modularity-optimal solutions.
...
```{r}
setwd("outputs/02_partII_CAMs/MultilayerExtraction")
