      ...
      $(function () {
        ...
        $("#submitAssoButton").on("click", () => {
          ...
          if (
            ...
            toastr.warning(
              "Please enter at least two letters for each word and then press 'Next response' or 'Enter'.",
              "Please provide all five associations that come to your mind.",
              {
                closeButton: true,
                timeOut: 3000,
...
const AffectiveImageryAffect_htmlForm = new lab.html.Page({
  ...
  messageHandlers: {
    run: function anonymous() {
      ...
      $("td.small:contains('entry1')").html(study.options.datastore.get(AIT_cue + '_' + 'R1'));
      $("td.small:contains('entry2')").html(study.options.datastore.get(AIT_cue + '_' + 'R2'));
      $("td.small:contains('entry3')").html(study.options.datastore.get(AIT_cue + '_' + 'R3'));
      $("td.small:contains('entry4')").html(study.options.datastore.get(AIT_cue + '_' + 'R4'));
      $("td.small:contains('entry5')").html(study.options.datastore.get(AIT_cue + '_' + 'R5'));
